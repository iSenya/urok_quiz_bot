...
class DBCommands:
    GET_CURRENT_QUEST_ID = (
        "SELECT quest_id FROM publications ORDER BY pubdate DESC LIMIT 1"
    )
    pool: Connection = db
    ADD_NEW_USER_REFERRAL = (
        "INSERT INTO users(user_id, username, full_name, referral) "
    ...
    CHECK_REFERRALS = (
        "SELECT user_id FROM users WHERE referral="
    ...
    GET_RANDOM_QUESTION = "SELECT body, topic, quest_id, (SELECT sum(points) FROM answers a WHERE q.quest_id = a.quest_id) as points FROM questions q ORDER BY points ASC LIMIT 1"
    ...
    CHECK_POINTS_EGE = (
        "SELECT COUNT(*) FROM answers WHERE user_id = $1 AND quest_id = $2"
    )
    ...
    async def check_referrals(self):
        ...
        return ", ".join(
            [
                f"{num + 1}. "
                + (await bot.get_chat(user["user_id"])).get_mention(as_html=True)
    ...
    async def save_published_question(self, chat_id, quest_id, message_id):
        publication = await self.pool.fetchrow(
            self.SAVE_PUBLISHED_QUESTION, chat_id, quest_id, message_id
        )
        return publication
...
async def stat(message: Message):
    ...
    for user in top_10:
        try:
            chat_user = (
                await types.Chat.get_current().get_member(user["user_id"])
            ).user
            user["username"] = chat_user.username
            user["last_name"] = chat_user.last_name
            user["first_name"] = chat_user.first_name
        except:
...
async def answer(message: Message):
    ...
    if is_answered:
        ...
        fuzz.ratio(user_answer.lower(), answer) >= 80
